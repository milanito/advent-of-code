...
const part1Solver = (entry:string):number => {
  console.log(entry)
...
if (require.main === module) {
  ...
  const inputFile = process.argv[2] || path.join(__dirname, "..", "inputs", "day14_test.txt");
  const input = fs.readFileSync(inputFile, "utf8");
  const mod = require("./day14");
